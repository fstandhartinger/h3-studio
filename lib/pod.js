...
// server.js registers a callback here rather than this module importing server.js, which
// would be a cycle. Called whenever the endpoint the app should talk to changes.
let onComfyUrl = () => {};
export function setComfyUrlSink(fn) { onComfyUrl = fn || (() => {}); }

let lastPublishedUrl = null;
function publishUrl(url) {
  if (url === lastPublishedUrl) return;
  lastPublishedUrl = url;
  try { onComfyUrl(url); } catch (e) { note(`could not switch ComfyUI endpoint: ${e.message}`); }
}
...
/** Does ComfyUI on this pod answer? The only question that decides whether it is usable. */
async function comfyAnswers(url) {
  if (!url) return false;
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 12000);
    try {
      const r = await fetch(`${url}/system_stats`, { signal: ctrl.signal });
      return r.ok;
...
export async function refresh() {
  ...
  if (!current || current.id !== mine.id) {
    ...
    publishUrl(current.comfyUrl);
  } else {
    current.costPerHr = Number(mine.costPerHr) || current.costPerHr;
    current.createdAt = createdAt;

    // The app's opinion yields to observable reality. Provisioning has several ways to
    // report failure while the installer actually finishes -- a dropped ssh, a launch
    // command that never returned -- and a pod stuck in 'error' with a working ComfyUI on
    // it is a pod being paid for and not used. If it answers, it is running.
    if (current.state === 'error' || current.state === 'provisioning') {
      if (await comfyAnswers(current.comfyUrl)) {
        note(`pod ${current.id} answers on ComfyUI; marking it running`);
...
async function provision(id, { chroma }) {
  ...
  setPhase('installing');
  // setsid + nohup so the installer outlives this ssh connection; it runs ~12 minutes and
  // a dropped connection must not take it down.
  //
  // Whether THIS ssh call returns is not evidence of anything. It was originally awaited,
  // and when the call did not return inside its timeout the whole provision was reported
  // as failed -- while the installer it had just started ran happily to completion on the
  // pod. A launch that is fire-and-forget by design must be confirmed by observing the
  // pod, not by the exit status of the command that lit the fuse.
  try {
    await ssh(ep, `chmod +x /root/setup.sh && WANT_CHROMA=${chroma ? 1 : 0} `
                + 'setsid nohup /root/setup.sh >/dev/null 2>&1 </dev/null & echo started', 25000);
  } catch (e) {
    note(`installer launch did not return cleanly (${e.message.slice(0, 80)}); `
       + 'checking whether it started anyway');
  }

  // It started if it wrote a phase. Give it a generous window: the pod's disk is network
  // storage and the first write can lag.
  let started = false;
  ...
  if (!started) throw new Error('the installer never started; see /workspace/setup.log on the pod');
  ...
  for (let i = 0; i < 240 && current?.id === id; i++) {
    ...
    if (phase === 'ready') {
      if (current?.id === id) {
        current.state = 'running';
        current.phase = 'ready';
        // Only now, not at creation: pointing the app at a pod whose ComfyUI is not up
        // yet just produces a stream of connection errors in the UI.
        publishUrl(current.comfyUrl);
      }
      note('pod is ready');
      emit({ type: 'ready', pod: status().pod });
